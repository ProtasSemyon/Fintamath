...
TEST(CotTests, callTest){
  EXPECT_EQ(Cot()(Rational(Constant("pi").toRational(10) / 2))->to<Rational>().toString(5), "-0");
  ...
  EXPECT_EQ(Cot()(Rational(Constant("pi").toRational(10) / 4))->to<Rational>().toString(5), "1");
